...
class Rigid2 {
 public:
  ...
  Rigid2(const Vector& translation, const double rotation) // rotation angle
      : translation_(translation), rotation_(rotation) {}
  ...
  Rigid2 inverse() const {
    const Rotation2D rotation = rotation_.inverse(); // R^-1
    const Vector translation = -(rotation * translation_); // -Rt
...
Rigid2<FloatType> operator*(const Rigid2<FloatType>& lhs,
  ...
  return Rigid2<FloatType>(
      lhs.rotation() * rhs.translation() + lhs.translation(), /* Rl*tr + tl*/
      lhs.rotation() * rhs.rotation());  /* Rl*Rr */
}

template <typename FloatType>
typename Rigid2<FloatType>::Vector operator*(
    const Rigid2<FloatType>& rigid,
    const typename Rigid2<FloatType>::Vector& point) /* [x y]^T */ {
...
std::ostream& operator<<(std::ostream& os,
                         const VINS::transform::Rigid2<T>& rigid) {
...
template <typename T>
std::ostream& operator<<(std::ostream& os,
                         const VINS::transform::Rigid3<T>& rigid) {
  os << rigid.DebugString();
  return os;
}
